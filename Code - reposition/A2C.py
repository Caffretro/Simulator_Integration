...
class A2C:
    def __init__(self, action_dim, state_dim, available_directions, load_model, model_name='', **kwargs):
        ...
        if self.load_model:
            self.actor.load_weights(load_path + "/save_model/1300/" + model_name + "_actor")
            self.critic.load_weights(load_path + "/save_model/1300/" + model_name + "_critic")
    ...
    def train_model(self, state_array, action_array, reward_array, next_state_array, done_array):
        ...
        if not np.all(~con_not_done):
            target_array[con_not_done, 0] = reward_array[con_not_done] + self.discount_factor * next_value_array[
                con_not_done]
        self.critic.fit(state_array, target_array, batch_size=batch_size, epochs=1, verbose=2)
        ...
        self.actor.fit(state_array, advantages_array, batch_size=batch_size, epochs=1, verbose=2)
